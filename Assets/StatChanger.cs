...
public class StatChanger : MonoBehaviour
{
    ...
    [SerializeField] List<StatChange> attributeChanges;

    public void ApplyAttributes(Entity entity)
    {
        if (!entity?.entityStats) return;
        EntityStats.Attribute attr;
        EntityStats.Stat stat;
        float v;
        foreach (StatChange ac in attributeChanges)
        {
            ...
            switch(ac.statType)
            {
                case StatType.Attribute_Base:
                    ...
                    v = attr.value_raw; 
                    break;
                case StatType.Attribute_Mod: v = attr.value_mod;
                    attr = entity.entityStats.GetSetAttribute(ac.statName, ac.defaultValue);
                    break;
